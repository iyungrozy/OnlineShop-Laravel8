...
<head>
  ...
  <title>@yield('title') | A&A Stores</title>
...
<nav class="navbar navbar-expand p-0 bg-success-darkest sticky-top">
 <a class="navbar-brand text-center col-xs-12 col-md-3 col-lg-2 mr-0" href="{{route('menu-masakan')}}">         <img src="{{url('polished/assets/ngapak.png')}}" alt="logo" width="120px"></a>
  ...
  @if(Auth::check() && Auth::user() && (Auth::user()->level == 'admin' || Auth::user()->level == 'owner'))
    <a class="navbar-brand text-right" href="{{route('admin.home')}}"><span class="oi oi-home"></span>Admin</a>
@endif
...
@else
    <a class="dropdown-item login" href="{{route('login')}}"><span class="oi oi-account-login"></span> Login</a>
@endif
    </div>
  </div>
</nav>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        Your Name<h3>      @if(Auth::check() && Auth::user())
          {{ Auth::user()->fullname }}
      @else
          Pengunjung
      @endif</h3>
        Your Username<h4>      @if(Auth::check() && Auth::user())
          {{ Auth::user()->username }}
      @else
          Pengunjung
      @endif</h4>
        Your Email<h4>      @if(Auth::check() && Auth::user())
          {{ Auth::user()->email }}
      @else
          Pengunjung
      @endif</h4>
        Your Level<h4>      @if(Auth::check() && Auth::user())
          {{ Auth::user()->level }}
      @else
          Pengunjung
      @endif</h4>
