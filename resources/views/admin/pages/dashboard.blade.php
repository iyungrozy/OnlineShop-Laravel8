            ...
            <div class="card-body">
                <div class="media">
                  <?php
                    ...
                    <a href="{{route('cashier')}}" class="text-decoration-none">
            ...
            <div class="card-body">
                <div class="media">
                  <?php
                    ...
                    <a href="{{route('admin.masakan')}}" class="text-decoration-none">
            ...
            <div class="card-body">
                <div class="media">
                  <?php
                    ...
                    <a href="{{route('admin.transaksi')}}" class="text-decoration-none">
    ...
    <div class="col-lg-7 pl-3 pt-2 pb-2 pr-2" style="background-image: url('polished/assets/main.png') !important; background-size: 80% ; background-repeat: no-repeat; background-position: 15rem -5rem">
     <div class="greeting mt-4 pl-4">
       ...
       <h4 class="text-muted w-50 fw-normal">
         Inilah yang terjadi pada layanan anda hari ini.
       </h4>
     </div>
...
<div class="col-lg-12 pt-3 mx-auto">
  <div class="card shadow-sm ">
    <div class="card-header bg-primary text-light d-inline">
      <h6><b>Data Transaksi per-bulan</b><a class="btn btn-success float-right ml-2" href="{{route('print.excel')}}"><span class="oi oi-print"></span> Export Excel</a><a class="btn btn-danger float-right" href="{{route('print.pdf')}}"><span class="oi oi-print"></span> Export PDF</a></h6>
    </div>
    <!-- Chart Tag -->
    <div class="card-body">
        <canvas id="chartjs-line-month"></canvas>
    </div>
    <div class="card-footer small text-muted">Grafik Line</div>
  </div>
</div>
...
while($x < 10) {
 ...
 <?php
$data_month = App\Transaksi::join('orders', 'transactions.order_id_order', '=', 'orders.id_order')
            ->select(DB::raw('MONTH(transactions.tanggal_transaksi) as month'), DB::raw('SUM(orders.subtotal) as total'))
            ->groupBy('month')
            ->get();
...
foreach ($data_month as $data) {
    $data_month_labels[] = date('F', mktime(0, 0, 0, $data->month, 1));
    $data_month_total[] = $data->total;
}
?>
<script type="text/javascript">
   var labelsMonth = <?= json_encode($data_month_labels) ?>;
   var totalMonth = <?= json_encode($data_month_total) ?>;

   var ctxLineMonth = document.getElementById('chartjs-line-month');
   var dataLineMonth = {
      labels: labelsMonth,
      datasets: [{
    ...
    var myLineChartMonth = new Chart(ctxLineMonth, {
      type: 'line', 
      data: dataLineMonth
    });
</script>
